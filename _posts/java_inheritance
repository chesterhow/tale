...
1. **<상속을 사용하게 된 이유>**는 ==중복 코드가 발생==했기 때문입니다. 해당 코드를 재사용하기 위하여 중복되는 내용들을 임의의 A 클래스에 묶고 이 코드가 필요한 모든 클래스가 A를 상속하여 필요한 메소드를 사용하도록 했습니다. 

2. 하지만 **<코드 재사용을 위해 상속을 사용하면 발생하는 문제점>**이 있었습니다. 처음에는 두어 번의 중복 코드 발생으로 이를 상위 클래스로 옮겼습니다. 그런데 상위 클래스를 상속하는 하위 클래스가 많아지면서 상위 클래스의 메소드가 제공하는 기능과 ==매우 유사하지만 완전히 일치하지는 않는 코드가 필요한 경우==가 생긴 것입니다. 이런 경우 해결 방법은 두 가지입니다.
